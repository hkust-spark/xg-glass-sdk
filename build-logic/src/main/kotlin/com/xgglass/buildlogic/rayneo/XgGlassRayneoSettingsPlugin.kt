 ...
 */
class XgGlassRayneoSettingsPlugin : Plugin<Settings> {
    override fun apply(settings: Settings) {
        val hostProjectPath = settings.providers.gradleProperty("xgRayneoHostProjectPath").orNull ?: ":xgglass_rayneo_glass_host"
        val hostProjectName = hostProjectPath.removePrefix(":")

        val root = settings.settingsDir
        val generatedRoot = File(root, "build/xgglass/generated")
    ...
    private fun generateHostIfNeeded(hostDir: File) {
        val marker = File(hostDir, ".xgglass_template_version")
