...
abstract class XgGlassRayneoExtension @Inject constructor(objects: ObjectFactory) {
    val hostProjectPath: Property<String> = objects.property(String::class.java).convention(":xgglass_rayneo_glass_host")
    val logicProjectPath: Property<String> = objects.property(String::class.java).convention(":xgglass_app_logic")
 ...
 */
class XgGlassRayneoAppPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        val ext = project.extensions.create("xgRayneo", XgGlassRayneoExtension::class.java)

        project.afterEvaluate {
            val hostPath = ext.hostProjectPath.get()
            val hostProject = project.rootProject.findProject(hostPath)
                ?: error("RayNeo host project not found: $hostPath. Did you apply com.xgglass.rayneo.settings in settings.gradle(.kts)?")
            ...
            require(mercuryDir.isNotBlank()) {
                "xgRayneo.mercuryAarDir is required to enable RayNeo UI/navigation (Mercury SDK). " +
                    ...
                    "Tried defaults: third_party/rayneo/aar, vendor/rayneo/aar, ../xg-glass-sdk/third_party/rayneo/aar, ../../xg-glass-sdk/third_party/rayneo/aar"
            }
    ...
    private fun findDefaultMercuryAarDir(project: Project): String? {
        ...
        // - in-repo development might keep vendor AARs under the SDK checkout: ../../xg-glass-sdk/third_party/rayneo/aar
        val root = project.rootProject.rootDir
        val candidates = listOf(
            File(root, "third_party/rayneo/aar"),
            File(root, "vendor/rayneo/aar"),
            File(root, "../xg-glass-sdk/third_party/rayneo/aar"),
            File(root, "../../xg-glass-sdk/third_party/rayneo/aar"),
            File(root, "vendor/rayneo/aar"),
    ...
    private fun patchHostFiles(hostDir: File, logicProjectPath: String, appEntryClass: String) {
        ...
        if (buildFile.exists()) {
            val updated = buildFile.readText()
                .replace("__XG_LOGIC_PROJECT__", logicProjectPath)
            buildFile.writeText(updated)
        }

        if (manifestFile.exists()) {
            val updated = manifestFile.readText()
                .replace("__XG_APP_ENTRY_CLASS__", appEntryClass)
