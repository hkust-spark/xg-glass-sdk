...
class ExampleAppEntry : UniversalAppEntrySimple {
    ...
    override fun commands(): List<UniversalCommand> {
        ...
        return listOf(CapturePhotoCommand)
    }
    private object CapturePhotoCommand : UniversalCommand {
        ...
        ctx.log("Capturing photo...")

        val result = ctx.client.capturePhoto(CaptureOptions())

        return result
            .onSuccess { image ->
                ctx.log("Photo captured (${image.jpegBytes.size} bytes).")
                ctx.onCapturedImage?.invoke(image)
            }
            .onFailure { e -> ctx.log("Capture failed: ${e.message}") }
            .map {} // convert Result<CapturedImage> → Result<Unit>
