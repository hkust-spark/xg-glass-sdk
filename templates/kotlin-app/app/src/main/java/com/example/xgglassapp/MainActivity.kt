...
class MainActivity : AppCompatActivity() {
    ...
    private val requestPermissionsLauncher =
            registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { result
                ...
                pendingConnectModel = null
                if (model != null) {
                    // For META: Android permissions are now satisfied, but we still need to
                    // verify registration and camera permission — re-enter the full flow.
                    if (model == GlassesModel.META) ensurePermissionsThenConnect(model)
                    else connect(model)
                }
            }
    ...
    private fun ensurePermissionsThenConnect(model: GlassesModel) {
        ...
        if (model == GlassesModel.META) {
            ...
            scope.launch {
                // ── Step 1: Registration ─────────────────────────────────────────────
                // Per DAT docs (permissions-requests): "Without registration, permission
                // requests fail." Always verify registration BEFORE requesting camera
                // permission, or the permission request will silently fail.
                try {
                    val regState = Wearables.registrationState.first()
                    if (regState !is RegistrationState.Registered) {
                        appendLog(
                                "Meta: not registered with the Meta AI app " +
                                        "(state: $regState). Opening registration flow — " +
                                        "complete it in the Meta AI app, then press Connect again."
                        )
                        tvStatus.text = "Status: Meta registration required"
                        pendingConnectModel = null
                        // Trigger the one-time registration UI inside the Meta AI app.
                        Wearables.startRegistration(this@MainActivity)
                        return@launch
                    }
                    appendLog("Meta: registered (state: $regState).")
                } catch (e: Exception) {
                    // Non-fatal: log and continue. The SDK will reject operations if
                    // truly unregistered.
                    appendLog(
                            "Meta: could not read registration state (${e.message}); " +
                                    "proceeding — SDK will reject if unregistered."
                    )
                }
                    ...
                    result
                            .onSuccess { status ->
                                if (status == PermissionStatus.Granted) {
                                    ...
                                    appendLog(
                                            "Meta: requesting camera permission " +
                                                    "(current: $status) — follow the Meta AI app prompt."
                                    )
